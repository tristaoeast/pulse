...
class SolrCloudStreams(solrService: SolrService) extends LazyLogging {
  ...
  val maxBuffersize: Int =
    sys.props.get("pulse.collector.buffer.max").map(_.toInt).getOrElse(DEFAULT_MAX_BUFFER_SIZE)
  val batchFlushDuration = {
    val property =
      sys.props
        .get("pulse.collector.solr.flush.seconds")
        .map(_.toInt)
  ...
  val solrBatchSize =
    sys.props.get("pulse.collector.solr.batch.size").map(_.toInt).getOrElse(DEFALUT_GROUP_SIZE)
  ...
  val groupedInsert: RunnableGraph[ActorRef] = {
    Source
      .actorRef[(String, LogEvent)](maxBuffersize, OverflowStrategy.dropNew)
      .groupBy(MAX_SUBSTREAMS, x => x._1) // group by the application name
      .groupedWithin(solrBatchSize, batchFlushDuration) // group 1000 records or within one second, whichever comes first
